// Lightweight audio synthesizer using Web Audio API for Canteen chime effects
class SoundEffects {
  ...
  // Pleasant bell chime for order placed / ready for pickup
  playOrderPlaced() {
  ...
  // Double chime for item added to cart
  playAddToCart() {
    if (!this.enabled) return;
    try {
      this.initCtx();
  ...
  playSuccess() {
    this.playOrderPlaced();
  }

  // Notification chime for kitchen call
  playKitchenCall() {
    if (!this.enabled) return;
    try {
      this.initCtx();
